...
ML_MODELS_RESOURCES_DIR = os.path.join(
    OFFLINE_PREPARATION_DIR, "ml_models_and_resources"
)
...
try:
    ...
    model = SentenceTransformer(
        embedding_model_name, cache_folder=ML_MODELS_RESOURCES_DIR
    )
    print(
        f"{embedding_model_name} 처리 완료. {ML_MODELS_RESOURCES_DIR} 내에 있는지 확인하십시오."
    )
...
print("\n--- ML 자산 다운로드 스크립트 완료 ---")
print(
    f"모든 다운로드된 자산은 다음 하위 디렉토리에 있어야 합니다: {ML_MODELS_RESOURCES_DIR}"
)
print(
    f"샌드박스 환경으로 './offline_preparation/' 내의 '{os.path.basename(ML_MODELS_RESOURCES_DIR)}' 디렉토리 전체를 전송해야 합니다."
)
