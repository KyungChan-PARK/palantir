...
async def test_user_creation_performance(async_client, metrics, db_session):
    """사용자 생성 성능 테스트."""
    # 테스트 데이터 준비
    users = [
        {
    ...
    for user in users:
        ...
        async with async_client.post(
            "http://localhost:8000/api/users/", json=user, headers=headers
        ) as response:
            metrics.stop()
...
async def test_user_retrieval_performance(async_client, metrics, db_session, test_user):
    """사용자 조회 성능 테스트."""
    # 토큰 생성
    token = create_access_token(
        {"sub": test_user["username"], "scopes": test_user["scopes"]}
    )
    ...
    for _ in range(1000):
        ...
        async with async_client.get(
            f"http://localhost:8000/api/users/{test_user['id']}", headers=headers
        ) as response:
            metrics.stop()
...
async def test_concurrent_user_operations(async_client, metrics, db_session):
    """동시 사용자 작업 성능 테스트."""
    # 테스트 데이터 준비
    users = [
        {
    ...
    async def create_user(user_data):
        async with async_client.post(
            "http://localhost:8000/api/users/", json=user_data, headers=headers
        ) as response:
...
async def test_authentication_performance(async_client, metrics):
    """인증 성능 테스트."""
    # 테스트 데이터
    ...
    for _ in range(1000):
        metrics.start()
        async with async_client.post(
            "http://localhost:8000/api/auth/token", data=auth_data
        ) as response:
            metrics.stop()
            assert response.status == 200
...
async def test_database_connection_pool(async_client, metrics, db_session):
    """데이터베이스 연결 풀 성능 테스트."""
    # 토큰 생성
    ...
    async def make_request():
        async with async_client.get(
            "http://localhost:8000/api/users/me", headers=headers
        ) as response:
            return response.status
...
async def test_memory_usage(async_client, metrics, db_session):
    """메모리 사용량 테스트."""
    import os
