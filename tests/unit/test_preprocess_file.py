...
IMG_BYTES = __import__("base64").b64decode(
    "iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAIAAAACUFjqAAAADklEQVQI12P4//8/AwAI/AL+dp1AOwAAAABJRU5ErkJggg=="
)
...
async def test_preprocess_file_image(monkeypatch):
    ...
    monkeypatch.setattr(
        "palantir.core.clip_embed.embed_image_clip", lambda img: [0] * 512
    )
...
async def test_preprocess_file_excel(monkeypatch):
    monkeypatch.setattr(
        "pandas.read_excel", lambda buf, engine=None: _pd.DataFrame({"a": [1]})
    )
    out = await pf.preprocess_file(
        "test.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        b"excel",
        "job5",
...
async def test_preprocess_file_raw():
    with pytest.raises(HTTPException) as excinfo:
        await pf.preprocess_file(
            "test.bin", "application/octet-stream", b"rawdata", "job6"
        )
    assert excinfo.value.status_code == 415
