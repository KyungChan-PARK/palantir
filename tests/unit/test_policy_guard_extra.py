...
class DummyRequest:
    def __init__(self, headers):
        ...
        self.url = "http://test"
        async def body(): return b"body"
        self.body = body

def test_user_tier_func_gold():
    token = jwt.encode({"tier": "gold"}, policy_guard.SECRET_KEY, algorithm=policy_guard.ALGORITHM)
...
def test_verify_jwt_valid():
    token = jwt.encode({"sub": "test"}, policy_guard.SECRET_KEY, algorithm=policy_guard.ALGORITHM)
