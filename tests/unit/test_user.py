...
def test_create_user(test_client, admin_user):
    """사용자 생성 테스트."""
    # 관리자 토큰 생성
    access_token = create_access_token(data={"sub": admin_user.username, "scopes": admin_user.scopes})
...
def test_get_current_user(test_client, test_user):
    """현재 사용자 정보 조회 테스트."""
    access_token = create_access_token(data={"sub": test_user.username, "scopes": test_user.scopes})
