...
async def test_password_security(async_client, test_user):
    ...
    for password in weak_passwords:
        response = await async_client.post(
            "/users/",
            headers={"Authorization": f"Bearer {create_access_token(data={'sub': 'admin', 'scopes': ['admin']})}"},
            json={
...
async def test_xss_prevention(async_client, test_user):
    ...
    for attempt in xss_attempts:
        response = await async_client.put(
            ...
            headers={"Authorization": f"Bearer {create_access_token(data={'sub': test_user.username, 'scopes': test_user.scopes})}"},
            json={
                "full_name": attempt,
