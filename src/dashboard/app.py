...
def execute_command(command: str, parameters: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    response = requests.post(f"{API_URL}/execute", headers=get_headers(), json=data)
    return response.json()
...
def register_agent(config: Dict[str, Any]) -> Dict[str, Any]:
    """에이전트 등록"""
    data = {"config": config}
    response = requests.post(f"{API_URL}/agents", headers=get_headers(), json=data)
    return response.json()
...
try:
    ...
    with col1:
        st.metric(
            ...
            (
                ...
                if status.get("active_agents") is not None
                else 0
            ),
        )
    with col2:
    ...
    with col3:
        ...
        st.metric(
            "메모리 사용량", f"{mem_usage:.1f}%" if mem_usage is not None else "0.0%"
        )
...
if st.button("자가 개선 diff/patch 프리뷰"):
    try:
        ...
        resp = requests.post(
            f"{API_URL}/self_improve/preview", headers=get_headers(), json=payload
        )
            ...
            with st.expander(f"{item['file']}"):
                ...
                with col2:
                    reason = st.text_input(
                        f"(선택) 승인/거부 사유 입력", key=f"reason_{i}"
                    )
                if approved_flag:
...
with st.form("rollback_form"):
    ...
    if st.form_submit_button("롤백 실행"):
        try:
            ...
            resp = requests.post(
                f"{API_URL}/self_improve/rollback", headers=get_headers(), json=payload
            )
            st.success("롤백 결과:")
            st.json(resp.json())
        except Exception as e:
...
try:
    ...
    try:
        df_si = pd.read_sql_query(
            "SELECT * FROM selfimprove_history ORDER BY timestamp DESC LIMIT 100",
            pipe.conn,
        )
        if not df_si.empty:
            df_si["status"] = df_si["diff"].apply(lambda d: "적용" if d else "실패")
            st.dataframe(
                df_si.style.applymap(
                    lambda v: (
            ...
            st.markdown(
                f"**최근 개선 건수:** {len(df_si)}  |  **최근 변경 파일:** `{df_si.iloc[0]['file']}` / {df_si.iloc[0]['timestamp']}"
            )
        else:
    ...
    try:
        ...
        if last_change is not None:
            st.markdown(
                f"**최근 변경 파일:** `{last_change['file']}` / {last_change['timestamp']}"
            )
        # 테스트 통과율
        ...
        try:
            ...
            log = subprocess.check_output(
                ["git", "log", "--oneline", "-5"], cwd=repo_path, encoding="utf-8"
            )
