...
class MCP:
    ...
    async def execute_command(self, command: str, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                self.logger.error(
                    f"명령어 실행 실패 (시도 {attempt + 1}/{self.config.max_retries}): {str(e)}"
                )
    ...
    async def register_agent(self, agent_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            f"{self.config.base_url}/agents", json=agent_config
        ) as response:
            response.raise_for_status()
            return await response.json()
    ...
    async def llm_generate(self, prompt: str, model: str = "gpt-4o", **kwargs) -> dict:
        ...
        if not openai_api_key:
            raise RuntimeError(
                "OpenAI API Key(OPENAI_API_KEY 환경변수)가 설정되어 있지 않습니다. 관리자에게 문의하세요."
            )
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": kwargs.get("system_prompt", "You are an AI agent."),
                },
