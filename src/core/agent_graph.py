...
planner = PlannerAgent(AgentConfig(name="Planner", description="Task Planner", model="gpt-4", system_prompt="계획 수립"))
developer = DeveloperAgent(AgentConfig(name="Developer", description="코드 생성", model="gpt-4", system_prompt="코드 생성"))
reviewer = ReviewerAgent(AgentConfig(name="Reviewer", description="코드 검토", model="gpt-4", system_prompt="코드 검토"))
selfimprover = SelfImprovementAgent(AgentConfig(name="SelfImprover", description="자가 개선", model="gpt-4", system_prompt="자가 개선"))
...
planner_tool = Tool(
    name="Planner",
    func=planner_func,
    description="Task를 분해하고 계획을 수립"
)
developer_tool = Tool(
    name="Developer",
    func=developer_func,
    description="코드를 생성/수정"
)
reviewer_tool = Tool(
    name="Reviewer",
    func=reviewer_func,
    description="코드/결과를 검토"
)
selfimprove_tool = Tool(
    name="SelfImprover",
    func=selfimprove_func,
    description="코드베이스 자가 개선"
)
