    ...
    async def apply_improvements(
        ...
        try:
            ...
            async with MCP(mcp_config) as mcp:
                ...
                branch_name = (
                    f"ai_suggestion_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
                # 브랜치 생성
                await mcp.git_op(
                    op="checkout",
                    repo_path=repo_path,
                    args={"branch": branch_name, "create": True},
                )
                # 변경 파일 add/commit
                await mcp.git_op(
                    op="add",
                    repo_path=repo_path,
                    args={"files": [patch.get("file", "") for patch in approved_files]},
                )
                commit_msg = f"AI 개선안 자동 커밋: {', '.join([patch.get('file', '') for patch in approved_files])}"
                await mcp.git_op(
                    op="commit", repo_path=repo_path, args={"message": commit_msg}
                )
                # 푸시
                await mcp.git_op(
                    op="push", repo_path=repo_path, args={"branch": branch_name}
                )
                # PR 생성(옵션)
                await mcp.git_op(
                    op="pr",
                    repo_path=repo_path,
                    args={"branch": branch_name, "title": commit_msg},
                )
        except Exception as e:
