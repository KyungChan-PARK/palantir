...
async def test_user_retrieval(session: aiohttp.ClientSession) -> float:
    ...
    for i in range(1000):
        async with session.get(
            f"http://localhost:8000/api/v1/users/{i % 100 + 1}"
        ) as response:
...
async def test_concurrent_users(session: aiohttp.ClientSession) -> float:
    ...
    tasks = []
    for i in range(50):
        tasks.append(session.get(f"http://localhost:8000/api/v1/users/{i % 100 + 1}"))
...
async def test_authentication(session: aiohttp.ClientSession) -> float:
    ...
    for i in range(1000):
        async with session.post(
            ...
            json={"username": f"testuser{i % 100}", "password": "testpassword123"},
        ) as response:
            assert response.status == 200
    return time.time() - start_time
...
async def main():
    ...
    try:
        ...
        console.print(
            f"[bold red]테스트 실행 중 오류가 발생했습니다: {str(e)}[/bold red]"
        )
        sys.exit(1)
