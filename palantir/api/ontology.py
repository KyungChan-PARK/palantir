...
from palantir.ontology.repository import OntologyRepository, embedding_node, query_similar_nodes, expand_related_nodes
...
def ontology_graph():
    ...
    return {"nodes": ["Customer", "Order", "Payment", "Delivery"], "edges": [("Customer", "Order"), ("Order", "Payment"), ("Order", "Delivery")]}
...
def similar_products(product_id: str):
    ...
    for p in products:
        if p['id'] != product_id:
            score = 0
            if target['properties'].get('category') and p['properties'].get('category') == target['properties'].get('category'):
                score += 1
            score += len(set(target['properties'].get('tags', [])) & set(p['properties'].get('tags', [])))
            if score > 0:
                sims.append((p, score))
