...
FEEDBACK_LOG = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../logs/feedback_log.csv")
)
...
def main():
    ...
    def fetch_metric_history_multi(ticker: str):
        try:
            ...
            try:
                ...
                shares = (
                    stock.get_income_stmt().loc["Basic Shares Outstanding", :].values[0]
                )
    ...
    with tab1:
        ...
        if not metrics or all(v is None for v in metrics.values()):
            st.warning(
                "지표 데이터를 불러올 수 없습니다. (티커 확인 또는 데이터 소스 점검 필요)"
            )
        else:
            try:
        ...
        if metric_hist is not None and not metric_hist.empty:
            ...
            if chart_data.shape[1] > 0 and chart_data.notnull().any().any():
                try:
                    ...
                    if st.session_state["highlight_date"]:
                        ...
                        if highlight_x in chart_data.index:
                            fig.add_trace(
                                go.Scatter(
                                    x=[highlight_x],
                                    y=[chart_data.loc[highlight_x, "PER"]],
                                    mode="markers",
                    ...
                    fig.update_layout(
                        clickmode="event+select", xaxis_title="날짜", yaxis_title="PER"
                    )
                ...
                month_select = st.selectbox(
                    "특정 월의 뉴스만 보기", ["전체"] + months, index=0
                )
                if month_select != "전체":
    ...
    with tab2:
        ...
        all_news = [
            (cat, entry)
            for cat, entries in news_by_category.items()
            for entry in entries
        ]
        keyword = st.text_input("뉴스 키워드 필터", value="")
        days = st.slider(
            "최근 N일 이내 뉴스만 보기", min_value=1, max_value=90, value=30
        )
        cutoff = datetime.now() - timedelta(days=days)
        sentiment_options = ["전체", "🟢 긍정", "🔴 부정", "⚪️ 중립"]
        sentiment_filter = st.selectbox(
            "감성(긍정/부정/중립) 필터", sentiment_options, index=0
        )
        ...
        if not news_by_category or all(len(v) == 0 for v in news_by_category.values()):
            ...
            for category, entries in news_by_category.items():
                ...
                if not filtered:
                    continue
                st.markdown(
                    f"**{category}** <span style='background-color:#eee;border-radius:8px;padding:2px 8px;'>{len(filtered)}</span>",
                    unsafe_allow_html=True,
                )
                for i, (title, summary, url, published, sentiment, pub_dt) in enumerate(
                    filtered
                ):
                    ...
                    def highlight(text, kw):
                        ...
                        return pattern.sub(
                            f"<mark style='background: #ffe066'>{kw}</mark>", text
                        )
                    ...
                    if st.button(
                        f"뉴스 날짜 차트에 표시: {title[:30]}...", key=btn_key
                    ):
                        if pub_dt:
                            st.session_state["highlight_date"] = pub_dt.strftime(
                                "%Y-%m-%d"
                            )
                    st.markdown(
                        f"- <a href='{url}' target='_blank'>{title_html}</a> <span style='font-size:0.9em'>{sentiment}</span>",
                        unsafe_allow_html=True,
                    )
                    if summary:
