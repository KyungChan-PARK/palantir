...
def render_search_section():
    ...
    if search_type == "Natural Language":
        ...
        with col1:
            obj_type = st.selectbox("Object Type", ["Customer", "Order", "Product"])

        with col2:
            property_field = st.text_input(
                "Property Filter (JSON)", placeholder='{"status": "pending"}'
            )

        if st.button("Search"):
            try:
                properties = json.loads(property_field) if property_field else None
                results = repo.search_objects(obj_type=obj_type, properties=properties)
                display_results(results)
        ...
        if filters:
            ...
            if st.button("Execute Query"):
                results = repo.search_objects(obj_type=obj_type, filters=filters)
                display_results(results)
...
def display_results(results: List[Dict]):
    ...
    with col2:
        if view_type == "Basic Charts":
            chart_type = st.selectbox(
                "Chart Type", ["Bar", "Line", "Scatter", "Pie", "Box", "Violin"]
            )
    ...
    if view_type == "Table":
        st.dataframe(
            ...
            column_config={"id": st.column_config.TextColumn("ID", width="medium")},
        )
        ...
        if len(df) > 0:
            try:
                ...
                y_col = st.selectbox(
                    "Y-axis", numeric_cols if numeric_cols else df.columns
                )
                if chart_type == "Bar":
        ...
        if any(df.dtypes.str.contains("datetime")):
            if st.checkbox("Show Time Series Analysis"):
                ...
                fig = px.line(
                    df, x=date_col, y=metric_col, title=f"{metric_col} Over Time"
                )
                st.plotly_chart(fig, use_container_width=True)
...
def render_filters_section():
    ...
    value_range = st.sidebar.slider(
        "Value Range", min_value=0, max_value=1000, value=(0, 1000)
    )
