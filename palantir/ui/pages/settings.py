...
class UISettings(BaseModel):
    ...
    default_visualization: str = Field("table", description="Default visualization type")
    ...
    with col1:
        analytics_settings["export_format"] = st.selectbox(
            ...
            index=["json", "csv", "excel"].index(
                analytics_settings["export_format"]
            ),
                ...
                with st.spinner("Preparing export..."):
                    try:
                        ...
                        if export_format == "JSON":
                            ...
                            data = pd.DataFrame({"sample": ["data"]}).to_csv(index=False)
                            mime = "text/csv"
                            ext = "csv"
                        else:  # Excel
                            data = pd.DataFrame({"sample": ["data"]}).to_excel(index=False)
...
def render_page():
    ...
    if st.button("Save All Settings"):
        try:
            ...
            settings["api"]["openai_api_key"] = encrypt_value(
                settings["api"]["openai_api_key"]
            ) if settings["api"]["openai_api_key"] else None
            
            settings["api"]["neo4j_password"] = encrypt_value(
                settings["api"]["neo4j_password"]
            ) if settings["api"]["neo4j_password"] else None
