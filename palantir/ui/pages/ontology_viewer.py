...
def render_object_details():
    ...
    if st.session_state.current_object:
        ...
        if obj['type'] == 'Order':
            ...
            timeline = [e for e in events if e['properties'].get('related_id') == obj['id']]
            timeline = sorted(timeline, key=lambda e: e['properties'].get('timestamp', ''))
            for e in timeline:
                st.write(f"{e['properties'].get('timestamp', '')}: {e['properties'].get('event_type', '')} - {e['properties'].get('description', '')}")
            if not timeline:
                st.info("이벤트 없음")
        # 실시간 상태/알림 예시
...
def render_page():
    ...
    if st.button("유사 객체 검색") and query:
        resp = requests.get(f"{API_URL}/ontology/similar", params={"q": query, "n": n_results})
    ...
    rec_n = st.number_input("추천 개수", min_value=1, max_value=10, value=3, key="rec_n")
    if st.button("추천 결과 보기") and rec_query:
        rec_resp = requests.get(f"{API_URL}/ontology/similar", params={"q": rec_query, "n": rec_n})
    ...
    events = [
        {"id": "e1", "type": "order_created", "timestamp": "2024-06-01T10:00:00", "description": "주문 생성"},
        {"id": "e2", "type": "payment_completed", "timestamp": "2024-06-01T10:05:00", "description": "결제 완료"},
        {"id": "e3", "type": "delivery_shipped", "timestamp": "2024-06-02T09:00:00", "description": "배송 출발"},
        {"id": "e4", "type": "delivery_delivered", "timestamp": "2024-06-03T15:00:00", "description": "배송 완료"},
    ]
