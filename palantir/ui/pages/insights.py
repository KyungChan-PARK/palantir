...
def render_object_insights():
    ...
    obj_type = st.selectbox("Select Object Type", ["Customer", "Order", "Product"])
    ...
    if st.button("Generate Insights"):
        with st.spinner("Analyzing objects..."):
            ...
            if analysis_type == "Basic Insights":
                ...
                # Aggregate insights
                all_key_points = [
                    point for insight in insights for point in insight.key_points
                ]
                ...
                for i in range(n_clusters):
                    with st.expander(f"Cluster {i} Analysis"):
                        cluster_objects = [
                            obj for j, obj in enumerate(objects) if clusters[j] == i
                        ]
...
def render_trend_analysis():
    ...
    time_range = st.selectbox(
        "Time Range", ["Last Week", "Last Month", "Last Quarter", "Last Year"]
    )

    # Metric selector
    metric = st.selectbox(
        "Metric", ["Order Volume", "Revenue", "Customer Growth", "Product Performance"]
    )
    ...
    if st.button("Analyze Trends"):
        ...
        with col2:
            st.metric("Average", f"{historical_data.mean():.2f}")

        with col3:
            growth_rate = (
                (current_value - historical_data.iloc[0])
        ...
        if show_seasonality:
            ...
            for component, values in zip(components, decomposition):
                fig.add_trace(
                    go.Scatter(x=historical_data.index, y=values, name=component)
                )
...
def render_relationship_insights():
    ...
    with col1:
        ...
        relationship_type = st.selectbox(
            "Relationship Type", ["Customer-Order", "Order-Product", "Customer-Product"]
        )
    ...
    if st.button("Analyze Relationships"):
        ...
        for source in source_objects[:5]:  # Limit for demo
            for target in target_objects[:5]:
                insight = processor.analyze_relationship(
                    source, target, relationship_type, depth=analysis_depth
                )
        ...
        for obj in source_objects + target_objects:
            G.add_node(
                str(obj["id"]), label=f"{obj['type']}: {obj.get('name', 'Unnamed')}"
            )
...
def render_page():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Object Insights", "Trend Analysis", "Relationship Insights"]
    )
