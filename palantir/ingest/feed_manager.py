...
CONFIG_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../config/rss_feeds.yaml'))
...
def fetch_and_parse_feeds() -> Dict[str, List[dict]]:
    ...
    for category, url in FEED_URLS.items():
        ...
        result = feedparser.parse(url, etag=etag_cache[category], modified=modified_cache[category], request_headers=HEADERS)
...
def filter_news_by_ticker(feeds_data: Dict[str, List[dict]], ticker: str) -> Dict[str, List[dict]]:
    ...
    for category, entries in feeds_data.items():
        for entry in entries:
            ...
            if ticker.upper() in title.upper() or ticker.upper() in summary.upper() or f'({ticker.upper()})' in title.upper():
