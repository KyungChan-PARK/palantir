...
def adjust_recommendation_by_news(base_rec: str, news_by_category: Dict[str, list]) -> str:
    """
    뉴스 이벤트(배당, 자사주, 가이던스, 부채 등)로 추천 보정
    """
    positive_events = news_by_category.get("Dividends") or news_by_category.get("Buybacks") or news_by_category.get("Guidance")
...
def recommend_with_news(metrics: Dict[str, float], news_by_category: Dict[str, list]) -> Tuple[str, str]:
