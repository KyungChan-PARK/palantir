    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"캐시 조회 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
        ...
        for attempt in range(self.max_retries):
            try:
                return self.redis.setex(
                    key, ttl or self.ttl, json.dumps(value), nx=nx, xx=xx
                )
            except (RedisError, TypeError) as e:
                logger.error(
                    f"캐시 저장 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
    ...
    async def delete(self, key: str) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"캐시 삭제 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
    ...
    async def clear(self) -> None:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"캐시 초기화 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
    ...
    async def exists(self, key: str) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"키 존재 확인 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
    ...
    async def get_many(self, keys: List[str]) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.error(
                    f"다중 캐시 조회 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                for key, value in mapping.items():
                    pipeline.setex(key, ttl or self.ttl, json.dumps(value))
                pipeline.execute()
                return
            except (RedisError, TypeError) as e:
                logger.error(
                    f"다중 캐시 저장 실패 (시도 {attempt + 1}/{self.max_retries}): {e}"
                )
                if attempt == self.max_retries - 1:
...
def get_cache(
    ...
    Returns:
        Cache 인스턴스
    """
    return Cache(ttl=ttl, host=host, port=port, db=db, password=password)
