...
def handle_csv(buffer):
    ...
    df = pd.read_csv(io.BytesIO(buffer) if isinstance(buffer,(bytes,bytearray)) else buffer)
    return df

def handle_json(buffer):
    import pandas as pd, json, io
    data = json.load(io.TextIOWrapper(io.BytesIO(buffer))) if isinstance(buffer,(bytes,bytearray)) else json.load(buffer)
...
async def preprocess_file(filename, mime, content, job_id):
    mime_token = detect_mime(mime)
    if mime.startswith("text/csv") or mime.startswith("application/vnd.ms-excel") or filename.endswith(('.csv', '.xls', '.xlsx')):
        df = pd.read_csv(io.BytesIO(content)) if filename.endswith('.csv') else pd.read_excel(io.BytesIO(content), engine='openpyxl')
